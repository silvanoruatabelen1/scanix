...
export default function AddProductModal({ isOpen, onClose, onProductAdded }: AddProductModalProps) {
  ...
  const handleSubmitV2 = () => {
    if (!formData.name || !formData.sku || !formData.category) {
      toast({ title: "Campos requeridos", description: "Completa nombre, SKU y categor�a", variant: "destructive" });
      return;
    }
    const basePrice = parseFloat(formData.basePrice);
    if (isNaN(basePrice) || basePrice <= 0) {
      toast({ title: "Precio inv�lido", description: "Ingresa un precio base v�lido", variant: "destructive" });
      return;
    }
    try {
      ...
      toast({ title: "Producto agregado", description: `${formData.name} se agreg� exitosamente` });
      onProductAdded();
      onClose();
    } catch (e) {
